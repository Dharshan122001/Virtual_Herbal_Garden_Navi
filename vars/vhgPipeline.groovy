  ...
  pipeline {
    ...
    stages {
      ...
      stage('Build and Push Images') {
        when {
          expression { return env.NO_DEPLOYABLE_CHANGES != 'true' }
        }
        steps {
          withCredentials([usernamePassword(
            ...
            sh '''#!/usr/bin/env bash
              ...
              build_backend_service() {
                ...
                docker buildx build --platform linux/amd64 -t "${DOCKERHUB_REPO}/${image_name}:${IMAGE_TAG}" --push .
                popd >/dev/null
                rm -rf "$workspace"
              }

              if [ "$PLANT_CHANGED" = "true" ]; then build_backend_service "plant_service" "plant-service"; fi
              if [ "$AUTH_CHANGED" = "true" ]; then build_backend_service "auth_service" "auth-service"; fi
              if [ "$AI_CHANGED" = "true" ]; then build_backend_service "ai_service" "ai-service"; fi

              if [ "$FRONTEND_CHANGED" = "true" ]; then
                pushd client >/dev/null
                docker buildx build --platform linux/amd64 -t "${DOCKERHUB_REPO}/frontend:${IMAGE_TAG}" --push .
                popd >/dev/null
              fi
            '''
          }
        ...
        steps {
          withCredentials([string(credentialsId: 'aks-kubeconfig', variable: 'KUBECONFIG_BASE64')]) {
            sh '''#!/usr/bin/env bash
              ...
              if helm list -n "$KUBE_NAMESPACE" -q | grep -qx "$HELM_RELEASE"; then RELEASE_EXISTS=true; fi

              HELM_ARGS=(helm upgrade --install "$HELM_RELEASE" "./$HELM_CHART_DIR" --namespace "$KUBE_NAMESPACE" --create-namespace --wait --timeout 10m)
              if [ "$RELEASE_EXISTS" = "true" ]; then HELM_ARGS+=(--reuse-values); fi

              if [ "$FRONTEND_CHANGED" = "true" ] || [ "$RELEASE_EXISTS" = "false" ]; then HELM_ARGS+=(--set "frontend.image=${DOCKERHUB_REPO}/frontend" --set "frontend.tag=${IMAGE_TAG}"); fi
              if [ "$PLANT_CHANGED" = "true" ] || [ "$RELEASE_EXISTS" = "false" ]; then HELM_ARGS+=(--set "plant.image=${DOCKERHUB_REPO}/plant-service" --set "plant.tag=${IMAGE_TAG}"); fi
              if [ "$AUTH_CHANGED" = "true" ] || [ "$RELEASE_EXISTS" = "false" ]; then HELM_ARGS+=(--set "auth.image=${DOCKERHUB_REPO}/auth-service" --set "auth.tag=${IMAGE_TAG}"); fi
              if [ "$AI_CHANGED" = "true" ] || [ "$RELEASE_EXISTS" = "false" ]; then HELM_ARGS+=(--set "ai.image=${DOCKERHUB_REPO}/ai-service" --set "ai.tag=${IMAGE_TAG}"); fi
